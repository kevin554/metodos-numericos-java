...
public class FormularioFalsaPosicion extends JPanel {
    ...
    private MetodoFalsaPosicion laEcuacion;
    ...
    public FormularioFalsaPosicion(Ventana laVentana) {
    ...
    private void init() {
        laEcuacion = new MetodoFalsaPosicion();
    ...
    private void despejar(ActionEvent evt) {
        ...
        while (matcher.find()) {
            ...
            sb.append(((MetodoFalsaPosicion) laEcuacion).
                    aplicarEcuacion(limiteInferior, limiteSuperior));
    ...
    private void obtenerDatos() {
        if (!tfError.getText().trim().equals("")) { // Si hay texto
            if (tfError.getText().trim().contains("%")) {
                ((MetodoFalsaPosicion) laEcuacion).setHayPorcentaje(true);
            } else {
                ((MetodoFalsaPosicion) laEcuacion).setHayPorcentaje(false);
            }
        }
        
        ((MetodoFalsaPosicion) laEcuacion).setIteraciones(0);
        ((MetodoFalsaPosicion) laEcuacion).setHayPorcentaje(true);
//        ((MetodoFalsaPosicion) laEcuacion).setErrorEstimado(Double.parseDouble(tfError.getText().replace("%", "")));
        ((MetodoFalsaPosicion) laEcuacion).setErrorPorcentual(100);
    ...
    public MetodoFalsaPosicion getLaEcuacion() {
        return laEcuacion;
    }

    public void setLaEcuacion(MetodoFalsaPosicion laEcuacion) {
